...
async function submit(env: Env, req: InvokeRequest<MotionBackendInput>): Promise<InvokeResponse<MotionBackendOutput>> {
  ...
  const audio = input.audio_url || (typeof req.config?.audio_url === "string" ? req.config.audio_url : "");
  if (!audio) {
    return { ok: false, error: "infinitetalk: needs Cast audio (audio_url) so the speaker stays ours" };
  }
  const route = await runpodRoute(env);
  if (!route.credential) return { ok: false, error: "infinitetalk: " + runpodCredentialName(route) + " not configured" };
  try {
    ...
    if (!r.ok) return { ok: false, error: "infinitetalk /run -> " + r.status };
    const jobId = ((await r.json()) as { id?: string }).id;
    if (!jobId) return { ok: false, error: "infinitetalk /run returned no job id" };
...
async function poll(env: Env, body: PollRequest): Promise<PollResponse<MotionBackendOutput>> {
  ...
  if (!st) return { ok: false, error: "infinitetalk: bad poll token" };
  const route = await runpodRoute(env);
  if (!route.credential) return { ok: false, error: "infinitetalk: " + runpodCredentialName(route) + " not configured" };
  ...
  if (runpodJobGone(httpStatus, s)) {
    ...
    if (classifyGoneState(st.submittedAt, now) === "gone-failed") {
      ...
      return { ok: false, error: "infinitetalk job not found on RunPod (GC'd or never ran); failing shot " + st.shotId + " (#141)" };
    }
    return { ok: true, pending: true };
  }
  if (s.status === "FAILED") {
    ...
    return { ok: false, error: "infinitetalk job failed: " + JSON.stringify(s.error ?? s).slice(0, 200) };
  }
  ...
  if (!url) return { ok: false, error: "infinitetalk output had no video url" };
  let bytes: ArrayBuffer;
  try {
    const v = await fetch(url);
    if (!v.ok) return { ok: false, error: "fetch infinitetalk video -> " + v.status };
    bytes = await v.arrayBuffer();
  } catch (e) {
    return { ok: false, error: "download infinitetalk video failed: " + (e as Error).message };
  }
