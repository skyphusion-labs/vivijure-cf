...
import { buildVeoBody, extractVideoUrl, clipKey, clampDuration, encodePoll, decodePoll, runpodJobGone, classifyGoneState, workersStillCold, terminalErrorInOutput, RUNPOD_COLD_GRACE_MS } from "./veo";
...
const ENDPOINT = "https://api.runpod.ai/v2/google-veo3-1-fast-i2v";
const OUT_FPS = 24;
...
async function submit(env: Env, req: InvokeRequest<MotionBackendInput>): Promise<InvokeResponse<MotionBackendOutput>> {
  ...
  if (!apiKey) return { ok: false, error: "google-veo: RUNPOD_API_KEY not configured" };
  try {
    const r = await fetch(ENDPOINT + "/run", {
      method: "POST",
      headers: { ...auth(apiKey), "content-type": "application/json" },
      body: JSON.stringify(buildVeoBody(input, req.config)),
    });
    if (!r.ok) return { ok: false, error: "google-veo /run -> " + r.status };
    const jobId = ((await r.json()) as { id?: string }).id;
    if (!jobId) return { ok: false, error: "google-veo /run returned no job id" };
    return {
      ok: true,
      pending: true,
    ...
    return { ok: false, error: "google-veo submit failed: " + (e as Error).message };
  }
}
...
async function poll(env: Env, body: PollRequest): Promise<PollResponse<MotionBackendOutput>> {
  ...
  if (!st) return { ok: false, error: "google-veo: bad poll token" };
  const apiKey = await secretValue(env.RUNPOD_API_KEY);
  if (!apiKey) return { ok: false, error: "google-veo: RUNPOD_API_KEY not configured" };
  ...
  if (runpodJobGone(httpStatus, s)) {
    ...
    if (classifyGoneState(st.submittedAt, now) === "gone-failed") {
      ...
      return { ok: false, error: "google-veo job not found on RunPod (GC'd or never ran); failing shot " + st.shotId + " (#141)" };
    }
    return { ok: true, pending: true };
  }
  if (s.status === "FAILED") return { ok: false, error: "google-veo job failed: " + JSON.stringify(s.error ?? s).slice(0, 200) };
  if (s.status !== "COMPLETED") {
    ...
    if (backendErr) {
      ...
      return { ok: false, error: "google-veo backend error (job " + st.jobId + ", status stuck " + String(s.status ?? "unknown") + ", cancel issued): " + backendErr };
    }
    return { ok: true, pending: true }; // IN_QUEUE / IN_PROGRESS
  }

  const url = extractVideoUrl(s.output);
  if (!url) return { ok: false, error: "google-veo output had no video url" };
  let bytes: ArrayBuffer;
  try {
    const v = await fetch(url);
    if (!v.ok) return { ok: false, error: "fetch google-veo video -> " + v.status };
    bytes = await v.arrayBuffer();
  } catch (e) {
    return { ok: false, error: "download google-veo video failed: " + (e as Error).message };
  }
