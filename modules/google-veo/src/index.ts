...
const ENDPOINT_ID = "google-veo3-1-fast-i2v";
const OUT_FPS = 24;
...
async function submit(env: Env, req: InvokeRequest<MotionBackendInput>): Promise<InvokeResponse<MotionBackendOutput>> {
  ...
  if (!route.credential) return { ok: false, error: "google-veo: " + runpodCredentialName(route) + " not configured" };
  try {
...
async function poll(env: Env, body: PollRequest): Promise<PollResponse<MotionBackendOutput>> {
  ...
  if (s.status === "FAILED") {
    ...
    return { ok: false, error: "google-veo job failed: " + JSON.stringify(s.error ?? s).slice(0, 200) };
  }
