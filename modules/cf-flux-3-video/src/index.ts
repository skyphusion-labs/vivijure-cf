// cf-flux-3-video: a motion.backend module worker (vivijure-module/2).
// Cloudflare AI Gateway / Unified Billing model `black-forest-labs/flux-3-video` (i2v).
//
...
export class CfFlux3VideoWorkflow extends WorkflowEntrypoint<Env, WorkflowParams> {
...
async function submit(
  ...
  const gatewayId = await secretValue(env.GATEWAY_ID);
  if (!gatewayId) return { ok: false, error: "cf-flux-3-video: GATEWAY_ID not configured" };
  ...
  try {
    ...
    return { ok: false, error: "cf-flux-3-video: could not start generation workflow: " + (e as Error).message };
  }
  ...
  try {
    ...
    return { ok: false, error: "cf-flux-3-video: could not persist run state: " + (e as Error).message };
  }
...
async function poll(env: Env, body: PollRequest): Promise<PollResponse<MotionBackendOutput>> {
  ...
  if (!token) return { ok: false, error: "cf-flux-3-video: bad poll token" };
  const state = await readState(env, token.job_id);
  if (!state) return { ok: false, error: "cf-flux-3-video: run state not found (expired or bad token)" };
