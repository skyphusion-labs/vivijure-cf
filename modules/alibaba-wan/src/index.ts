...
async function submit(env: Env, req: InvokeRequest<MotionBackendInput>): Promise<InvokeResponse<MotionBackendOutput>> {
  ...
  if (!route.credential) return { ok: false, error: "alibaba-wan: " + runpodCredentialName(route) + " not configured" };
  try {
...
async function poll(env: Env, body: PollRequest): Promise<PollResponse<MotionBackendOutput>> {
  ...
  if (s.status === "FAILED") {
    ...
    return { ok: false, error: "alibaba-wan job failed: " + JSON.stringify(s.error ?? s).slice(0, 200) };
  }
  ...
  if (s.status !== "COMPLETED") {
    ...
    if (backendErr) {
      ...
      return { ok: false, error: "alibaba-wan backend error (job " + st.jobId + ", status stuck " + String(s.status ?? "unknown") + ", cancel issued): " + backendErr };
    }
