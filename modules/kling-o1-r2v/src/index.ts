// kling-o1-r2v: a motion.backend module worker (vivijure-module/2), Kuaishou Kling V2.1 I2V Pro on RunPod.
...
const ENDPOINT_ID = "kling-video-o1-r2v";
const OUT_FPS = 24;
...
const MANIFEST: ModuleManifest = {
  ...
  provides: [{ id: "i2v-cloud", label: "Cast, props, locations (Kling O1)" }],
  config_schema: {
  ...
  ui: {
    ...
    locality: "cloud",
    cost: "Pay per render",
    blurb: "Multi-ref silent clips. 3-10 seconds. Holds cast, props, and places.",
    limits: [
      "3-10 second clips",
      "Silent motion",
...
async function submit(env: Env, req: InvokeRequest<MotionBackendInput>): Promise<InvokeResponse<MotionBackendOutput>> {
  ...
  if (!route.credential) return { ok: false, error: "kling-o1-r2v: " + runpodCredentialName(route) + " not configured" };
  try {
    ...
    if (!r.ok) return { ok: false, error: "kling-o1-r2v /run -> " + r.status };
    const jobId = ((await r.json()) as { id?: string }).id;
    if (!jobId) return { ok: false, error: "kling-o1-r2v /run returned no job id" };
...
async function poll(env: Env, body: PollRequest): Promise<PollResponse<MotionBackendOutput>> {
  ...
  if (!st) return { ok: false, error: "kling-o1-r2v: kling-o1-r2v: bad poll token" };
  const route = await runpodRoute(env);
  if (!route.credential) return { ok: false, error: "kling-o1-r2v: " + runpodCredentialName(route) + " not configured" };
  ...
  if (runpodJobGone(httpStatus, s)) {
    ...
    if (classifyGoneState(st.submittedAt, now) === "gone-failed") {
      ...
      return { ok: false, error: "kling-o1-r2v job not found on RunPod (GC'd or never ran); failing shot " + st.shotId + " (#141)" };
    }
    return { ok: true, pending: true };
  }
  if (s.status === "FAILED") {
    ...
    return { ok: false, error: "kling-o1-r2v job failed: " + JSON.stringify(s.error ?? s).slice(0, 200) };
  }
  ...
  if (!url) return { ok: false, error: "kling-o1-r2v output had no video url" };
  let bytes: ArrayBuffer;
  try {
    const v = await fetch(url);
    if (!v.ok) return { ok: false, error: "fetch kling-o1-r2v video -> " + v.status };
    bytes = await v.arrayBuffer();
  } catch (e) {
    return { ok: false, error: "download kling-o1-r2v video failed: " + (e as Error).message };
  }
