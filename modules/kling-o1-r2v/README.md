...
image-to-video backend, run on RunPod (`kling-video-o1-r2v`). It turns one shot's start keyframe
...
**Model / endpoint**: fixed in code -- `ENDPOINT = https://api.runpod.ai/v2/kling-video-o1-r2v`.
...
- **Async**: cloud i2v takes minutes, longer than a Worker request can hold. `POST /invoke` submits
  ...
  finds it). Bound into the core as `MODULE_KLING_O1_R2V`.
