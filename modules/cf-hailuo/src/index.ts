// cf-hailuo: a motion.backend module worker (vivijure-module/2).
// Cloudflare AI Gateway / Unified Billing model `minimax/hailuo-2.3` (i2v).
//
...
const MANIFEST: ModuleManifest = {
  ...
  provides: [{ id: "i2v-cloud", label: "Silent physical (Hailuo 2.3)" }],
  config_schema: {
    resolution: { type: "enum", values: ["768P", "1080P"], default: "768P", label: "resolution" },
    prompt_optimizer: { type: "bool", default: true, label: "expand prompt" },
    fast_pretreatment: { type: "bool", default: false, label: "fast pretreatment" },
  },
  ui: {
    section: "motion",
    order: 65,
    locality: "cloud",
    cost: "Pay per render",
    blurb: "Physical motion on Cloudflare. Silent. 6 or 10 seconds.",
    limits: [
...
export class CfHailuoWorkflow extends WorkflowEntrypoint<Env, WorkflowParams> {
...
async function submit(
  ...
  const gatewayId = await secretValue(env.GATEWAY_ID);
  if (!gatewayId) return { ok: false, error: "cf-hailuo: GATEWAY_ID not configured" };
  ...
  try {
    ...
    return { ok: false, error: "cf-hailuo: could not start generation workflow: " + (e as Error).message };
  }
  ...
  try {
    ...
    return { ok: false, error: "cf-hailuo: could not persist run state: " + (e as Error).message };
  }
...
async function poll(env: Env, body: PollRequest): Promise<PollResponse<MotionBackendOutput>> {
  ...
  if (!token) return { ok: false, error: "cf-hailuo: bad poll token" };
  const state = await readState(env, token.job_id);
  if (!state) return { ok: false, error: "cf-hailuo: run state not found (expired or bad token)" };
