// cf-hh1-r2v: a motion.backend module worker (vivijure-module/2).
// Cloudflare AI Gateway / Unified Billing model `alibaba/hh1.1-r2v` (i2v).
//
...
export class CfHh1R2vWorkflow extends WorkflowEntrypoint<Env, WorkflowParams> {
...
async function submit(
  ...
  const gatewayId = await secretValue(env.GATEWAY_ID);
  if (!gatewayId) return { ok: false, error: "cf-hh1-r2v: GATEWAY_ID not configured" };
  ...
  try {
    ...
    return { ok: false, error: "cf-hh1-r2v: could not start generation workflow: " + (e as Error).message };
  }
  ...
  try {
    ...
    return { ok: false, error: "cf-hh1-r2v: could not persist run state: " + (e as Error).message };
  }
...
async function poll(env: Env, body: PollRequest): Promise<PollResponse<MotionBackendOutput>> {
  ...
  if (!token) return { ok: false, error: "cf-hh1-r2v: bad poll token" };
  const state = await readState(env, token.job_id);
  if (!state) return { ok: false, error: "cf-hh1-r2v: run state not found (expired or bad token)" };
