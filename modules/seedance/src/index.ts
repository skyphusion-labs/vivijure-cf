...
const ENDPOINT_ID = "seedance-v1-5-pro-i2v";
const OUT_FPS = 24;
...
const MANIFEST: ModuleManifest = {
  ...
  ui: { section: "motion", order: 10, locality: "cloud", cost: "Pay per render", blurb: "Fast talking clips. Hosted default for speed." },
};
...
async function submit(env: Env, req: InvokeRequest<MotionBackendInput>): Promise<InvokeResponse<MotionBackendOutput>> {
  ...
  if (!route.credential) return { ok: false, error: "seedance: " + runpodCredentialName(route) + " not configured" };
