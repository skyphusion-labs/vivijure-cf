...
type VpcCorr = { filmKey?: string; project?: string; contextJobId?: string };
 ...
 *  cf#396: every hop is timed + logged (async_submit). */
async function submitAsync(
  ...
  const timed = await timedVpcFetch(
    ...
    { method: "POST", headers: await mediaFinishHeaders(env.MEDIA_FINISH_TOKEN), body: JSON.stringify(spec) },
    {
      ...
      contextJobId: corr.contextJobId,
    },
  );
  if (timed.err || !timed.resp) return null;
  if (timed.resp.status !== 202) return null;
  try {
    const body = (await timed.resp.json()) as { ok?: boolean; jobId?: string };
...
async function invokeSync(
  ...
  if (timed.err || !timed.resp) return passthrough(input, "passthrough:container-unreachable", true);
  ...
  // cf#396: wall-clock tag on success so film job applied history carries fleet spend duration.
  const applied = withVpcElapsedApplied(["film-titles"], timed.elapsedMs);
  return {
    ok: true,
    output: {
...
async function invoke(env: Env, req: InvokeRequest<FilmFinishInput>): Promise<InvokeResponse<FilmFinishOutput>> {
  ...
  if (jobId) {
    return { ok: true, pending: true, poll: encodePoll({ jobId, filmKey: input.film_key, outputKey: input.output_key, submittedAt: Date.now(), notFoundStreak: 0, titleSeconds }) };
  }
  return invokeSync(env, input, spec, titleSeconds, corr);
}
...
async function poll(env: Env, body: PollRequest): Promise<PollResponse<FilmFinishOutput>> {
  ...
  if (resp.status === 404) {
    ...
    logVpcAsyncTerminal({
      ...
      pollElapsedMs: timed.elapsedMs,
      httpStatus: 404,
      containerJobId: st.jobId,
      filmKey: st.filmKey,
    });
    return { ok: false, error: "film-titles: video-finish container job not found (no replica holds it); resubmit" };
  }
  if (!resp.ok) return { ok: true, pending: true }; // 5xx gateway blip: re-poll
  let s: { status?: string; result?: { ok?: boolean; key?: string; durationSeconds?: number } | null; error?: string };
  ...
  if (s.status === "completed") {
    ...
    const output = completedOutput(s.result, st);
    return { ok: true, output: { ...output, applied: withVpcElapsedApplied(output.applied, jobMs) } };
  }
  if (s.status === "failed") {
    logVpcAsyncTerminal({
      module: MANIFEST.name,
