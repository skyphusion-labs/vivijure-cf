// Pure MiniMax Hailuo 2.3 Fast mapping/parsing: build the RunPod request body, parse the result
...
// per-module so the module stays independent (matches the seedance reference).
 ...
 *  enable_prompt_expansion and go_fast default ON (the #171 snippet's defaults). */
export function buildHailuoBody(input: MotionBackendInput, cfg: Record<string, unknown>): {
  ...
  return {
    input: {
      ...
      enable_prompt_expansion: cfg.enable_prompt_expansion !== false,
      go_fast: cfg.go_fast !== false,
    },
  };
}
...
export function clipKey(project: string, shotId: string): string {
  ...
  return `renders/${safe(project)}/clips/${safe(shotId)}_hailuo.mp4`;
}
