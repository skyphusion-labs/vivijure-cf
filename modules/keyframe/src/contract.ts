...
export interface ModuleManifest {
   ...
   *  attaches the tenant's per-job R2 credential to its invoke envelope. OPTIONAL/additive, mirrors
...
export interface InvokeRequest<I = unknown> {
   ...
   *
   * THE ONLY SECRET THIS ENVELOPE CARRIES. `context` remains secrets-free by construction -- this is
   * a SIBLING of it, not a member. A receiver must call `takeTenantR2(req)` at the parse boundary,
   * which reads and REMOVES it in one step so nothing downstream holds an object containing it.
   *
   * Absent means ABSENT: the key is omitted, never null. The backend REFUSES an explicit null rather
   * than reading it as "use the environment".
   */
  r2?: TenantR2Config;
}
