...
// container's video-finish route over VIDEO_FINISH_URL (public HTTPS origin).
//
...
async function invokeSync(
  ...
  return {
    ...
    output: {
      film_key: filmKey,
      applied: withVpcElapsedApplied(applied, timed.elapsedMs),
      ...durationFields(body),
    },
  };
}
...
async function poll(env: Env, body: PollRequest): Promise<PollResponse<FilmFinishOutput>> {
  ...
  if (!vfBase(env)) return { ok: false, error: "subtitle: no VIDEO_FINISH_URL" };
