"""cf#571: a hung ffmpeg/ffprobe cannot hold an audio-master worker thread forever.
...
hits = _unbounded_subprocess_runs(os.path.join(here, "master_core.py"))
check("master_core.py has no unbounded subprocess.run", hits == [])

if check.failed:
    print(f"\n{check.failed} FAILED")
    sys.exit(1)
print("\nall audio-master ffmpeg-timeout checks passed")
