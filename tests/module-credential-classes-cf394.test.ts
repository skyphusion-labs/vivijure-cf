...
const CLASSIFICATION: Readonly<Record<string, { path: TenantPath; note: string }>> = {
  ...
  AUDIO_MASTER_URL: { path: "operator-only", note: "public HTTPS origin for the audio-master CPU container" },
  FINISH_DOOR_TOKEN: { path: "operator-only", note: "cf#480: the upscale door's own bearer (LOCAL_FINISH_TOKEN)" },
  SPEECH_DOOR_TOKEN: { path: "operator-only", note: "cf#480: the speech door's own bearer" },
  FINISH_DOOR_TOKEN_PROPAGANDHI: { path: "operator-only", note: "cf#507: second upscale door bearer" },
  SPEECH_DOOR_TOKEN_PROPAGANDHI: { path: "operator-only", note: "cf#507: second speech door bearer" },
  BLENDER_DOOR_TOKEN: { path: "operator-only", note: "cf#489: blender door bearer" },
  AUDIO_BEAT_SYNC_URL: { path: "operator-only", note: "public HTTPS origin for the audio-beat-sync CPU container" },
  VIDEO_FINISH_URL: { path: "operator-only", note: "public Traefik SUBMIT origin for video-finish" },
  FINISH_UPSCALE_DOORS: { path: "operator-only", note: "comma-separated HTTPS origins for on-iron upscale doors" },
  SPEECH_UPSCALE_DOORS: { path: "operator-only", note: "comma-separated HTTPS origins for on-iron speech doors" },
  FINISH_BLENDER_DOORS: { path: "operator-only", note: "comma-separated HTTPS origins for on-iron blender doors" },
  MEDIA_FINISH_TOKEN: { path: "operator-only", note: "cf#613/#615: bearer the fleet media containers check. Reuses the FINISH_DOOR_TOKEN store secret. Unset is fail-open" },
