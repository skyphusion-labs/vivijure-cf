...
situation that produced the false pass on vivijure-control-plane's #242 -- the bug cf never had a
guard to reproduce, since cf carried no entry-required check at all before cf#510/cf#539.
...
cf#539 EXTENSION. `changelog.d/` fragments (one file per PR, assembled into CHANGELOG.md at
release) are the fix for the PR queue being quadratic on a shared changelog anchor -- see
...
check("a containers-only PR needs no entry (own GHCR image cadence, not this repo's v* line)",
      cer.verdict(["containers/image-prep/requirements.txt"])[0])
...
check("a public/ change needs an entry too", not cer.verdict(["public/app.js"])[0])
check("a modules/ change needs an entry too", not cer.verdict(["modules/finish-upscale/index.ts"])[0])
...
# fix is additive and does not break every currently-open PR that already carries a CHANGELOG.md
# edit (cf#539's own stated constraint: 15 open PRs, none of which carry a fragment).
