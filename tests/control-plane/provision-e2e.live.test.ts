...
import { CfApi } from "../../src/control-plane/cf-api";
import { CfTokenMinter } from "../../src/control-plane/token-minter";
import { runProvisionJob, teardownTenant, type ProvisionDeps } from "../../src/control-plane/provisioner";
import { createTenantEndpoints, RunPodClient, tenantEndpointName, PROVISION_PLAN } from "../../src/control-plane/runpod";
...
describe.skipIf(!LIVE)("full provisioner chain (real CF + real RunPod scratch)", () => {
  ...
  it("provisions a REAL tenant end to end and parks at awaiting_invoke_key", async () => {
    ...
    if (!result.ok) throw new Error(`provision failed at ${result.step}: ${result.message}`);
