...
(function () {
  ...
  // cf#336: stage the bytes, then REGISTER by copy.
  //
  // POST /api/upload returns an `uploads/<uuid>.<ext>` key. The cast routes' JSON `{ key, mime }`
  // form does NOT accept that: it requires a key already staged under `cast/<internal id>/`
  // (requireCastStagedKey, added by f239532), so every file upload on this page answered
  // 400 "key must be a safe staged path under this cast member" from 2026-07-23 until this fix.
  //
  // The fix is the COPY form, and it is the stricter path rather than a workaround. `{ key, mime }`
  // passes no bytes, so the studio can only allowlist the CLAIMED mime; `from_chat_artifact`
  // fetches the object, requires its magic bytes to match the claimed type, and writes the result
  // under `cast/<id>/` itself. Widening the guard to accept `uploads/` would have re-opened exactly
  // what f239532 closed, on the door that serves these objects back out.
  //
  // The argument name predates the upload route: it copies from ANY studio artifact key, not only a
  // `chat` output. Returns the key ALONE, so no caller can rebuild the body the studio refuses.
  ...
  async function trainWanLora() {
    ...
    try {
      // cf#349: the button is labeled Wan; send model_family explicitly (mirror SDXL
      // path). Empty body used to inherit server default, which is wan only when
      // RUNPOD_WAN_TRAIN_ENDPOINT_ID is set -- otherwise it silently trained SDXL.
      const data = await api("/api/cast/" + id + "/train-lora", {
        method: "POST",
        headers: { "content-type": "application/json" },
